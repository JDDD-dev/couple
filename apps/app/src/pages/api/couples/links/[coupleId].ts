...
export default async function handlerGetLinks(req: NextApiRequest, res: NextApiResponse): Promise<void>{
    ...
    if (session){
        ...
        res.status(200).send(JSON.stringify(queryRes[0].coupleLinkSafe))
    }else{
        res.status(403).send({
            error: "Unauthorized"
        })
